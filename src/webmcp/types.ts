...
/**
 * Wraps fetch for tool handlers. `credentials: 'same-origin'` is the whole
 * security story here: the browser attaches JobJam's HttpOnly Supabase
 * cookie, the handler runs under that session's RLS identity, and the agent
 * never sees a token. Never add an Authorization header to this.
 */
